...
//--------------------初始化和公用方法 start ------------------------
	...
	function reLoadGridData(queryObj){
		...
		TouristRoamingOvernightInAction.getTouristRoamingOvernightInList(queryObj,function(result){
			...
			TouristRoamingOvernightInAction.getTouristRoamingOvernightInList2(queryObj,
					function(result1) {
				       result = eval('(' + result1 + ')'); 
						if (result.resultCode == "0") {
							loadData(result.returnList);
	...
	function downloadExcel(){
	    var number ="1";
		TouristRoamingOvernightInAction.expExcel2(queryFormCach,number,function(result){
			if(result.resultCode=="0"){
