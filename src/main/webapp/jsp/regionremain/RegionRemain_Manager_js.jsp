...
//	{field:'zertoonehCount',title:'0-1小时',width:100},
                {field: 'onetotwohCount', title: '0-2小时（人次）', width: 100},
                {field: 'twotosixhCount', title: '2-6小时（人次）', width: 100},
                {field: 'sixhtodayCount', title: '6-24小时（人次）', width: 100},
                {field: 'daytotwodCount', title: '24-48小时（人次）', width: 100},
                {field: 'moretwodCount', title: '48小时以上（人次）', width: 100},
    ...
    function loadData(data) {
        ...
        if (data != null && data.length > 0) {
            $('#RegionRemainList').datagrid({loadFilter:pagerFilter}).datagrid('loadData', {total:data.length,rows:data}).datagrid('getPager').pagination('select',1)
        } else {
    ...
    function levelChange1(){
			...
			var aLevel = document.getElementById("RegionRemainQueryForm:aLevel").value;
			//removeAllOptions();//删除已有的option
			$('#RegionRemainQueryForm\\:regionId option:selected').text("");
			$("#RegionRemainQueryForm\\:regionId").empty();
			//$("#regionId").val("");
			if (countyid=='') {
				return;
			}
