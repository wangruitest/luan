...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#RegionRealtimeSumAddList").datagrid({
            ...
            columns: [[
                ...
                {field: 'dates', title: '新增游客总量（人次）', width: 100},
    ...
    function downloadExcel() {
        ...
        RegionRealtimeSumAddAction.expExcel(queryObj, number,function (result) {
    ...
    function loadData(data) {
        ...
        if (data != null && data.length > 0) {
            $('#RegionRealtimeSumAddList').datagrid({loadFilter:pagerFilter}).datagrid('loadData', {total:data.length,rows:data}).datagrid('getPager').pagination('select',1);
        } else {
    ...
    function levelChange(){
	 ...
	 function levelChange1(){
        ...
        var regionId = document.getElementById("RegionRealtimeSumAddQueryForm:regionId");
        //removeAllOptions();//删除已有的option
        $('#RegionRealtimeSumAddQueryForm\\:regionId option:selected').text("");
        $("#RegionRealtimeSumAddQueryForm\\:regionId").empty();
        if (cityid=='') {
            return;
        }
        if(aLevel==''){
