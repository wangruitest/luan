...
<body style="height: 100%;">
	...
	<script type="text/javascript">
		...
		//图表在iframe中自适应显示
		// var mh = $(".main").height();
		var hh = parent.window.$("#chinamap2").height();
		$(".main").height(hh);
		...
		function getAhmapTouristInfor() {
			...
			var longitude = 31.7470318307;//城市或区县经度 116.5005646164,31.7470318307
			var latitude = 116.5005646164;//城市或区县纬度
			var simple_name = '六安';//城市名或区县名简称
			...
			$.ajax({
				type: "post",
				async: false, //同步执行
				url: basePath2 + "/bigScreen/getYesterdayTopTenCityTouristCountEntireCountry.do",
				// data: {
				// 	areacode: areacode,
				// 	flag: flag
				// },
				...
				success: function (result) {
					if (result.length > 0) {
						var yesterdayVisitors = result;
						alltourists = 0;
						for (var i = 0; i < yesterdayVisitors.length; i++) {
							alltourists += yesterdayVisitors[i].TOURIST;
						}
						for (var i = 0; i < yesterdayVisitors.length; i++) {
