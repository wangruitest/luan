...
/** 导出显示
*/
function showExport(pObject,dateParams,tObject,headerStr,title){
	var searchType = document.getElementById("searchtype").value;
	var timeType = null;
	var timeType = getTimeType(searchType);//获取时间类型
	var str = [];
...
/**生成导出excel报表*/
function toExportChart(pObject,tObject,chartType,headerStr,title){
	 var searchType = pObject.searchType;
	 var dateParams = [];
	 var chartTypes = tObject.chartTypes;
	 var types= chartTypes.split(",");
	 ...
	 showExport(pObject,dateParams,tObject,headerStr,title); //显示图
}
