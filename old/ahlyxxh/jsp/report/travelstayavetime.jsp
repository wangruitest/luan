...
<head>
    ...
    <script>
        ...
        $(function () {
            ...
            $("#gridbox").datagrid({
                ...
                columns: [[
                    ...
                    {field: 'AVGTIMESTR', title: '平均驻留时间', width: 100},
         ...
         * */
        function exportJc() {
            var headerStr = ["景区名称", "平均驻留时间", "统计时间"];
            var title = "平均驻留时间统计";
            var pObj = getSearchForm();
            if(!valid(pObj)) return;
            pObj.diff = getDiff(pObj);
        ...
        function getStaticObject() {
            ...
            tObject.subTitle = "统计全市所有景区的游客平均驻留时间，可按年、季度、月、天统计，游客可按全部游客、外地游客、本地游客分别统计。";
...
<div id="parameter" style="background-color: #E3EFFF;width:100%">
    ...
    <div >
        ...
        <span style="display: none;" id="ySearch">
            ...
            <label>结束时间:</label>
            <input type="text"  id="yEndTime" name="yEndTime" class="Wdate" value="<%=today %>" onclick="WdatePicker({dateFmt:'yyyy',maxDate:'#F{$dp.$D(\'yBeginTime\',{y:10})}',minDate:'#F{$dp.$D(\'yBeginTime\')}'});" />
        </span>
        <span style="display: none;" id="qSearch">
            <label>开始时间:</label>
            <input type="text" id="qBeginTime" name="qBeginTime" class="Wdate" value="<%=today %>" onclick="WdatePicker({dateFmt:'yyyy',minDate:'#F{$dp.$D(\'qEndTime\',{y:-1})}',maxDate:'#F{$dp.$D(\'qEndTime\')||\'%y\'}'});" />
            ...
            <label>结束时间:</label>
            <input type="text"  id="qEndTime" name="qEndTime" class="Wdate" value="<%=today %>" onclick="WdatePicker({dateFmt:'yyyy',maxDate:'#F{($dp.$D(\'qBeginTime\',{y:1}))}',minDate:'#F{$dp.$D(\'qBeginTime\')}'});" />
        ...
        <span  id="mSearch">
            <label>开始时间:</label>
            <input type="text"  id="mBeginTime" name="mBeginTime" class="Wdate" value="<%=month %>" onclick="WdatePicker({dateFmt:'yyyy-MM',minDate:'#F{$dp.$D(\'mEndTime\',{M:-10})}',maxDate:'#F{$dp.$D(\'mEndTime\')||\'%y-%M\'}'});" />
            <label>结束时间:</label>
