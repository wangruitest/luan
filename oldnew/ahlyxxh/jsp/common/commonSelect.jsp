...
<script>
	 ...
	 * 2.js调用 commonSelect(options,selectUsersBack);
	                参数说明：
	     (1)options={
		        type: 数据类型---必填，其中  01-选择人员,02-选择角色,03-选择预警人员
	       		title: 窗口标题---默认:'通用选择框',
	   		    width: 窗口宽度---默认:700,
	   		    height: 窗口高度---默认:700,
	   		    shadow: 窗口阴影---默认:true,
	...
	function loadCommonSelectData(options){
		...
		//加载系统用户
		if(options.type=="01"){
			SysOperatorAction.getSysOperatorList(null,function(result){
			...
			RegionAlarmLevelAction.queryRegions(function(result){
				if(result.resultCode=='0'){
					$(result.returnList).each(function(index,obj){
						var value=obj.regionId;
						var text=obj.regionName;
						if(selectedUnids.indexOf(value+";")<0){
							$("#common_toSelect").append("<option value='"+value+"'>"+text+"</option>");
						}
					});
