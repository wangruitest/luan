	...
	$(function(){
		...
		$("#RegionCfgList").datagrid({
			...
			onDblClickRow: function(rowIndex, rowData){
				...
				//初始化数据
				$("#RegionCfgForm").setJSON({RegionCfgForm:rowData});
...
//--------------------初始化和公用方法 start ------------------------
	...
	function loadData(data){
		...
		if(data!=null && data.length>0){
			$('#RegionCfgList').datagrid({loadFilter:pagerFilter}).datagrid('loadData', {total:data.length,rows:data});
		}else{
...
//--------------------界面操作 start ------------------------
	...
	//	loadJingquSelect('RegionCfgForm');
	//	$("#RegionCfgForm\\:typeid").combobox("setValue",rows[0]['typeid']);
/*		$('#Select_CellcfgList').datagrid('loadData',{total:0,rows:[]});
		var cellrow = rows[0].cellCfgList;
		for (var p in cellrow) {
			$('#Select_CellcfgList').datagrid('appendRow',cellrow[p]);
		}*/
		$('#RegionCfgDialog').dialog({
			title:'修改景区配置',
			modal:true,
