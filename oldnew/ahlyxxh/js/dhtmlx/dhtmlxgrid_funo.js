...
var globalActiveDHTMLGridObject;String.prototype._dhx_trim=function(){return this.replace(/&nbsp;/g, " ").replace(/(^[ \t]*)|([ \t]*$)/g, "")};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()), dhtmlxArray._master)};dhtmlxArray._master={_dhx_find:function(pattern){for (var i = 0;i < this.length;i++){if (pattern == this[i])return i};return -1},
 ...
 var cr = parseInt(xml.doXPath("//"+this.xml.top)[0].getAttribute("pos")||0);var total = parseInt(xml.doXPath("//"+this.xml.top)[0].getAttribute("total_count")||0);this.total_record = parseInt(xml.doXPath("//"+this.xml.top)[0].getAttribute("total_record")||0);if (total&&!this.rowsBuffer[total-1])this.rowsBuffer[total-1]=null;if (this.isTreeGrid())
 ...
 }};if (this._srnd&&!this._fillers)this._fillers=[this._add_filler(max, this.rowsBuffer.length-max)];this.setSizes();this.loadPagingToolBar();if(this.loadxml_callback.length>0)eval(this.loadxml_callback+"(this)");},
...
//分页开始，luojz
dhtmlXGridObject.prototype.is_paging=false;
dhtmlXGridObject.prototype.paging_div="";
dhtmlXGridObject.prototype.total_record=0;
dhtmlXGridObject.prototype.page_count=20;
dhtmlXGridObject.prototype.current_page=1;
dhtmlXGridObject.prototype.query_form=document;
dhtmlXGridObject.prototype.paging_class_name="";
dhtmlXGridObject.prototype.paging_callback="";
dhtmlXGridObject.prototype.loadxml_callback="";
dhtmlXGridObject.prototype.setIs_paging=function (is_paging1){this.is_paging=is_paging1;}
dhtmlXGridObject.prototype.setPageCount=function (page_count1){this.page_count=page_count1;}
dhtmlXGridObject.prototype.setQuery_form=function (query_form1){this.query_form=query_form1;}
dhtmlXGridObject.prototype.setCurrent_page=function (current_page1){this.current_page=current_page1;if(this.paging_callback.length>0){eval(this.paging_callback+"("+current_page1+")");}}
dhtmlXGridObject.prototype.setPaging_callback=function (paging_callback1){this.paging_callback=paging_callback1;}
dhtmlXGridObject.prototype.setLoadxml_callback=function (loadxml_callback1){this.loadxml_callback=loadxml_callback1;}
dhtmlXGridObject.prototype.setPaging_class_name=function (paging_class_name1){this.paging_class_name=paging_class_name1;}

dhtmlXGridObject.prototype.setPaging_div=function (paging_div1){
  this.paging_div=paging_div1;
  
  var barObj=document.getElementById(this.paging_div);
  
  //如果不分页，则把DIV外围的TR隐藏起来
  if(!this.is_paging){
    if(barObj!=null)barObj.parentNode.parentNode.style.display="none";
    return;
  }
  if(barObj==null){
    alert("The div name:"+this.paging_div+" does not exist");
    return;
  }
  var mygrid_obj=this;
  
  var table_bar=document.createElement("TABLE");
  barObj.appendChild(table_bar);
  
  var newTR = table_bar.insertRow(0);
  
  var cells1=newTR.insertCell(0);
  var cells1A=document.createElement("A");
  cells1A.href="#";
  cells1.appendChild(cells1A);
  var cells1PageFirst=document.createElement("IMG");
  cells1PageFirst.border="0";
  cells1PageFirst.title="第一页";
  cells1PageFirst.disabled=true;
  cells1PageFirst.src=this.imgURL+"/page-first-disabled.gif";
  cells1PageFirst.onclick=function(){if(this.disabled)return;mygrid_obj.setCurrent_page(1);mygrid_obj.sendQuery()};
  cells1A.appendChild(cells1PageFirst);
  
  var cells2=newTR.insertCell(1);
  var cells2A=document.createElement("A");
  cells2A.href="#";
  cells2.appendChild(cells2A);
  var cells2PagePrev=document.createElement("IMG");
  cells2PagePrev.border="0";
  cells2PagePrev.title="上一页";
  cells2PagePrev.disabled=true;
  cells2PagePrev.src=this.imgURL+"/page-prev-disabled.gif";
  cells2PagePrev.onclick=function(){if(this.disabled)return;mygrid_obj.setCurrent_page(mygrid_obj.current_page-1);mygrid_obj.sendQuery()};
  cells2A.appendChild(cells2PagePrev);
  
  var cells3=newTR.insertCell(2);
  cells3.innerHTML="第";
  
  var cells4=newTR.insertCell(3);
  var cells4Text=document.createElement("INPUT");
  cells4Text.type="text";
  cells4Text.style.width="40";
  cells4Text.style.textAlign="right";
  cells4Text.disabled=true;
  cells4Text.value="1";
  cells4Text.onkeyup=function keyup1(evt){
    ...
    if(keyCode==13){
      var total_page=Math.floor(mygrid_obj.total_record/mygrid_obj.page_count)+(mygrid_obj.total_record%mygrid_obj.page_count==0?0:1);
      var go_page_text=this.value;
      if(!isInt(go_page_text)){
        this.select();
        return;
      }
      var go_page_value=parseInt(go_page_text);
      if(go_page_value<1||go_page_value>total_page){
        this.select();
        return;
      }
      mygrid_obj.setCurrent_page(go_page_value);
      mygrid_obj.sendQuery();
    }
  };
  cells4.appendChild(cells4Text);
  
  var cells5=newTR.insertCell(4);
  cells5.innerHTML="页";
  
  var cells6=newTR.insertCell(5);
  var cells6A=document.createElement("A");
  cells6A.href="#";
  cells6.appendChild(cells6A);
  var cells6PageGo=document.createElement("IMG");
  cells6PageGo.border="0";
  cells6PageGo.title="到指定页数/刷新";
  cells6PageGo.disabled=true;
  cells6PageGo.src=this.imgURL+"/refresh.gif";
  
  cells6PageGo.onclick=function(){
    if(this.disabled)return;
    var total_page=Math.floor(mygrid_obj.total_record/mygrid_obj.page_count)+(mygrid_obj.total_record%mygrid_obj.page_count==0?0:1);
    var go_page_text=cells4Text.value;
    if(!isInt(go_page_text)){
      cells4Text.select();
      return;
    }
  ...
  cells6A.appendChild(cells6PageGo);
  
  var cells7=newTR.insertCell(6);
  var cells7A=document.createElement("A");
  cells7A.href="#";
  cells7.appendChild(cells7A);
  var cells7PageNext=document.createElement("IMG");
  cells7PageNext.border="0";
  cells7PageNext.title="下一页";
  cells7PageNext.disabled=true;
  cells7PageNext.src=this.imgURL+"/page-next-disabled.gif";
  
  cells7PageNext.onclick=function(){if(this.disabled)return;mygrid_obj.setCurrent_page(mygrid_obj.current_page+1);mygrid_obj.sendQuery()};
  cells7A.appendChild(cells7PageNext);
  
  var cells8=newTR.insertCell(7);
  var cells8A=document.createElement("A");
  cells8A.href="#";
  cells8.appendChild(cells8A);
  var cells8PageLast=document.createElement("IMG");
  cells8PageLast.border="0";
  cells8PageLast.title="最后一页";
  cells8PageLast.disabled=true;
  cells8PageLast.src=this.imgURL+"/page-last-disabled.gif";
  cells8PageLast.onclick=function(){if(this.disabled)return;var total_page=Math.floor(mygrid_obj.total_record/mygrid_obj.page_count)+(mygrid_obj.total_record%mygrid_obj.page_count==0?0:1);mygrid_obj.setCurrent_page(total_page);mygrid_obj.sendQuery()};
  cells8A.appendChild(cells8PageLast);
  
  var cells9=newTR.insertCell(8);
  cells9.nowrap=true;
  cells9.innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;总共 0 页&nbsp;&nbsp;&nbsp;当前第 0 页&nbsp;&nbsp;&nbsp;每页 0 条记录";
}

dhtmlXGridObject.prototype.loadPagingToolBar=function(){
  if(!this.is_paging)return;
  
  if(this.page_count<=0){
    alert("The number of records per page must be greater than zero");
    return;
  }
  ...
  //开始分页计算
  //alert(this.total_record%this.page_count);  //这行不能少啊，要不然IE会报错的，超级变态
  var total_page=Math.floor(this.total_record/this.page_count)+(this.total_record%this.page_count==0?0:1);
  ...
  if(this.current_page==1){
    barObj.children[0].rows[0].cells[0].children[0].children[0].disabled=true;
    barObj.children[0].rows[0].cells[0].children[0].children[0].src=this.imgURL+"/page-first-disabled.gif";
    barObj.children[0].rows[0].cells[1].children[0].children[0].disabled=true;
    barObj.children[0].rows[0].cells[1].children[0].children[0].src=this.imgURL+"/page-prev-disabled.gif";
  }else{
    barObj.children[0].rows[0].cells[0].children[0].children[0].disabled=false;
    barObj.children[0].rows[0].cells[0].children[0].children[0].src=this.imgURL+"/page-first.gif";
    barObj.children[0].rows[0].cells[1].children[0].children[0].disabled=false;
    barObj.children[0].rows[0].cells[1].children[0].children[0].src=this.imgURL+"/page-prev.gif";
  }
  //最后一页
  if(this.current_page==total_page){
    barObj.children[0].rows[0].cells[6].children[0].children[0].disabled=true;
    barObj.children[0].rows[0].cells[6].children[0].children[0].src=this.imgURL+"/page-next-disabled.gif";
    barObj.children[0].rows[0].cells[7].children[0].children[0].disabled=true;
    barObj.children[0].rows[0].cells[7].children[0].children[0].src=this.imgURL+"/page-last-disabled.gif";
  }else{
    barObj.children[0].rows[0].cells[6].children[0].children[0].disabled=false;
    barObj.children[0].rows[0].cells[6].children[0].children[0].src=this.imgURL+"/page-next.gif";
    barObj.children[0].rows[0].cells[7].children[0].children[0].disabled=false;
    barObj.children[0].rows[0].cells[7].children[0].children[0].src=this.imgURL+"/page-last.gif";
  }
  barObj.children[0].rows[0].cells[3].children[0].value=this.current_page;
  if(total_page>1){
    barObj.children[0].rows[0].cells[3].children[0].disabled=false;
    barObj.children[0].rows[0].cells[5].children[0].children[0].disabled=false;
  }else{
    barObj.children[0].rows[0].cells[3].children[0].disabled=true;
    barObj.children[0].rows[0].cells[5].children[0].children[0].disabled=false;
  }
  barObj.children[0].rows[0].cells[8].innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;总共 "+total_page+" 页&nbsp;&nbsp;&nbsp;当前第 "+this.current_page+" 页&nbsp;&nbsp;&nbsp;每页 "+this.page_count+" 条记录";
};
dhtmlXGridObject.prototype.getElementNameAndValue=function(){
  var name_value_list="";
  var inputElement = this.query_form.getElementsByTagName("INPUT");
  if (inputElement!=null) {
    for (i=0; i<inputElement.length; i++){
      //如果disabled或名称为空，则下一个
      if(inputElement.item(i).disabled||inputElement.item(i).name=="")continue;
      
      var type1=inputElement.item(i).type;
      if(type1=="hidden"||type1=="text"){
        name_value_list += "&"+inputElement.item(i).name+"="+inputElement.item(i).value;
      }
      if(type1=="radio"){
        if(inputElement.item(i).checked){
          name_value_list += "&"+inputElement.item(i).name+"="+inputElement.item(i).value;
        }
      }
    }
  }
  var selectElement = this.query_form.getElementsByTagName("SELECT");
  if (selectElement!=null) {
    for (ii=0;ii<selectElement.length;ii++){
      //如果disabled或名称为空，则下一个
      if(selectElement.item(ii).disabled||selectElement.item(ii).name=="")continue;
      
      name_value_list += "&"+selectElement.item(ii).name+"="+selectElement.item(ii).value;
    }
  }
  return name_value_list;
};
dhtmlXGridObject.prototype.getPagingServletPath=function(){
  ...
  postPath=postPath+"/dhtmlxgrid?is_paging="+this.is_paging+"&page_count="+this.page_count+"&current_page="+this.current_page+"&paging_class_name="+this.paging_class_name;
  
  return(prePath+postPath);
};
dhtmlXGridObject.prototype.sendQuery=function(){
  var pagingUrl=this.getPagingServletPath();
  pagingUrl=pagingUrl+this.getElementNameAndValue();
  this.clearAll(false);
  this.loadXML(pagingUrl);
};
function isInt(str) {
   var strPat=/^[0-9]*$/;
   if (!strPat.test(str)) {
