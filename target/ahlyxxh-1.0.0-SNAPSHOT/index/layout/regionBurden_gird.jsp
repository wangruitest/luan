...
<body style="height: 100%">
	...
	var imgIcons=['<%=request.getContextPath()%>/daping/images/green.png','<%=request.getContextPath()%>/daping/images/blue.png',
		...
		$(document).ready(function() {
			...
			*/
			if('<%=curCountyCode %>' != null && '<%=curCountyCode %>'!='' &&'<%=curCountyCode %>' !='0' && '<%=curCountyCode %>' != 'null'){
						 UtilsAction.getMyCitycode(function(data){//必须通过action添加option，不然会被移除，原因未知。
							$("#cityList").append("<option value='<%=curCountyCode %>'><%=curCountyName %></option>");
							reginChangeCity('<%=curCountyCode %>');
						});
			}else{
		...
		function reginChangeCity(cityCode){
			...
			var myRegionIds="";
			UtilsAction.getMyRegionCfgByCounty(cityCode,null,function(data){
				if(null==data || null==data.returnList)return;
				for (var i=0;i<data.returnList.length;i++) {
	...
	<div style="width:600px;height:120px;position:absolute; background:rgba( 169,169,169,0.8);filter:progid:DXImageTransform.Microsoft.gradient(startColorstr=#66000000,endColorstr=#66000000);left:81%; top:30px; z-index:9999">
	<lable><img src="<%=request.getContextPath()%>/daping/images/green.png" width="17" height="17" />：表示达到景区预设的绿色预警阈值</lable></br>
	<lable><img src="<%=request.getContextPath()%>/images/red.png" width="17" height="17" />：表示达到景区预设的红色预警阈值</lable></br>
	<lable><img src="<%=request.getContextPath()%>/daping/images/orange.png" width="17" height="17" />：表示达到景区预设的橙色预警阈值</lable></br>
	<lable><img src="<%=request.getContextPath()%>/daping/images/yellow.png" width="17" height="17" />：表示达到景区预设的黄色预警阈值</lable></br>
	<lable><img src="<%=request.getContextPath()%>/daping/images/blue.png" width="17" height="17" />：表示达到景区预设的蓝色预警阈值</lable></br>
	</div>
